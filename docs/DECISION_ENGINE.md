...
```text
collect files → intent/scope → AI findings → calibrate confidence
...
## 6. Interaction with Stage 3 intent

Intent/scope does **not** invent severity. It feeds decision when:

- Implementation is unrelated to the task  
- Scope creep or exceeds stated scope  
- Coverage is low  

Missing-work warnings appear in **Intent & Scope**; high missing work can influence coverage → review.
